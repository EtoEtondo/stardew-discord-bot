...
class FishCog(commands.Cog):
    ...
    @app_commands.command(name="fish", description="Get quick info about a fish")
    async def fish(
        self,
        ...
        """Send a fish embed with wiki link and a ⭐ reaction for feedback."""
        ...
        if not name:
            ...
            return
        url = wiki_service.build_fish_url(name, locale=locale)
        description = self.translator.translate("fish.response", locale=locale, name=name, url=url)
        embed = discord.Embed(
            title=f"🐟 {name.title()}",
